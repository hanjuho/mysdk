...
namespace hsdk
{
	...
	template<typename DATA, typename USE, typename KEY = std::wstring> DECL_INTERFACE(EMBEDDED_ARCHITECTURE_PATTERN)
	{
		...
		*/
		...
		$ 참고1 : PTR_SOFTWARE _software 파라미터에 값이 왔을 때, Software::AddRef를 호출할 것을 권장.
		$ 참고2 : 1번이 충족된 경우 사용이 끝난 _software는 반드시 Software::Release를 호출할 것을 권장.
		*/
